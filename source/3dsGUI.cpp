	...
	for (int i = 0;i < (int) txt.length(); i++) {
		...
		try {
			if (code >= 8252 && code <= 129510) {
				for (int i = 0;i < (int)emojiVec.size(); i++) {
...
void D3DSGUI::drawBottomScreen()
{
	try {
		...
		if (discordPtr->currentGuild != 0) {
			...
			sftd_draw_text(fontText, 74, 50, bottom_screen_info_colour, 13, discordPtr->guilds[discordPtr->currentGuild].channels[discordPtr->currentChannel].topic.c_str());
			...
			sftd_draw_text(fontText, 74, 50, bottom_screen_info_colour, 13, discordPtr->directMessages[discordPtr->currentChannel].recipients[0].username.c_str());
...
D3DSGUI::D3DSGUI(){
	...
	emojix = 16;
	emojiy = 16;
	for (int i = 0;i < (int) j_emojispritesheettable.size(); i++) {
		emoji tmpemoji;
		try {
...
{
	for (int i = 0;i < (int) avatarVec.size(); i++)
...
std::string D3DSGUI::wrap(const char * text, sftd_font *font, int font_size, size_t line_length)
{
	...
	if (words >> word) {
		...
		while (words >> word) {
			if ((int)space_left < sftd_get_text_width(font, font_size, word.c_str()) + font_size) {
				wrapped << '\n' << word;
...
void D3DSGUI::Draw(){
	...
	if(state == 0){
		...
		for (int i = 0;i < (int) guildBoxes.size(); i++) {
			if (guildBoxes[i].x + guildBoxes[i].w > 0 && guildBoxes[i].x < 240) {
				...
				sftd_draw_text(fontGuilds, guildScrollX + guildXIndex + 10, guildScrollY + i * 50 + 14, server_channel_text_colour, 22, discordPtr->guilds[i].name.c_str());
			}
		}
		...
		for(int i = 0 ;i < (int) channelBoxes.size() ; i++){
			if (channelBoxes[i].x + channelBoxes[i].w > 0 && channelBoxes[i].x < 240) {
				sftd_draw_text(fontGuilds, channelScrollX + channelXIndex + 10, channelScrollY + i * 50 + 14, server_channel_text_colour, 22, discordPtr->currentGuild == 0 ? discordPtr->directMessages[i].recipients[0].username.c_str() : ("#" + discordPtr->guilds[discordPtr->currentGuild].channels[i].name).c_str());
			}
		}
		...
		for (int i = 0;i < (int) messageBoxes.size(); i++) {
			if (messageBoxes[messageBoxes.size() - i - 1].y + messageBoxes[messageBoxes.size() - i - 1].h > 0 && messageBoxes[messageBoxes.size() - i - 1].y < 240) {
				if (discordPtr->currentGuild != 0) {
					...
					sftd_draw_text(fontUser, messageScrollX + 70, messageBoxes[messageBoxes.size() - i - 1].y + 9, avatarVec[uInfo].colour, 14, dispname.c_str());
					...
					sftd_draw_text(fontText, messageScrollX + 70, messageBoxes[messageBoxes.size() - i - 1].y + 32, message_text_colour, 13, discordPtr->directMessages[discordPtr->currentChannel].messages[messageBoxes.size() - i - 1].content.c_str());
					//sftd_draw_wtext(fontText, messageScrollX + 70, messageBoxes[messageBoxes.size() - i - 1].y + 32, message_text_colour, 13, discordPtr->guilds[discordPtr->currentGuild].channels[discordPtr->currentChannel].messages[messageBoxes.size() - 1 - i].content.c_str());
				}
				else {
...
int D3DSGUI::click(int x , int y){
	if(state == 0){
		for(int i = 0 ;i < (int) loginInputs.size() ; i++){
...
void D3DSGUI::setGuildBoxes(){
	guildBoxes.clear();
	for(int i = 0;i < (int) discordPtr->guilds.size() ; i++){
		box boxG;
